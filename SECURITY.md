...
For binary overrides, enforcement scales with how protected the bootstrap host itself is. When the bootstrap host is owned by the invoking user (the default per-user install), user-level malware could modify the bootstrap directly, so restricting binary ownership would add friction without benefit; any executable binary is therefore accepted. When the bootstrap host is installed system-wide such that the invoking user cannot modify it, overrides must resolve to root-owned binaries that the user cannot write, and the bootstrap refuses to start otherwise; tools without an override (`gpg`, `jq`, `openssl` defaults resolved via `PATH`) are held to the same bar, so a user-writable `PATH` component cannot shadow them. This is defence-in-depth against user-level malware when Parcel itself is installed somewhere the user lacks write access to; it is not a protection against full system compromise. A system-wide install (`parcel-setup.sh --system`) is recommended for this stronger guarantee.

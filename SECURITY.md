...
Compromised signing keys can be revoked without manual intervention by users. The main host script ships a `BLACKLIST_SIGNERS` list (space-separated primary or subkey fingerprints, empty unless a key has been revoked) and persists it to the state file (the same one used for rate-limiter state) at startup and on every state save. The bootstrap host refuses to load a shipped script that has no non-blacklisted valid signer, and combines the persisted list from the state file with any `BLACKLIST_SIGNERS` set in `parcelrc`, so a revocation rides the usual extension update path (AMO / Chrome Web Store). An unusable state file (invalid content or bad permissions) is repaired by the main host at startup rather than left in place, so a later bootstrap session enforces the shipped blacklist instead of failing open.
...
Once the host has filtered entries, the extension applies a second layer of visibility control per entry class. Each class has `originBound` and `scope` defaults that govern when entries are offered in the popup: `originBound: true` classes (the default for logins and passkeys) are only shown on matching origins, which is the primary anti-phishing mechanism. The `card` class defaults to `originBound: false` (visible on any origin) and `scope: "context"` (only shown when the page has card form fields). Both properties can be overridden per-rule in `.parcel.json`.
...
The WebAuthn interception script necessarily runs in each page's JavaScript realm (this is how WebAuthn works for any extension-based authenticator), which means sites can observe that an extension handles passkey requests - an extension of the detectability noted in the tradeoffs below. The script is inert on load: it wraps `navigator.credentials` only after the isolated content script has confirmed passkeys are enabled globally and in scope for the frame's URL, so pages where Parcel does not handle passkeys see a completely unmodified API. The events bridging this script to Parcel's isolated content script are ordinary DOM `CustomEvent`s, which page script can forge or observe; no shared secret between the two realms is possible, because anything the page realm can hold, a compromised page can read. Parcel therefore treats the bridge as untrusted: the origin and relying-party ID are re-derived and validated in the isolated world and background worker on every request, and every ceremony requires the consent popup displaying the true origin. A forged bridge message can summon an unwarranted popup, but cannot obtain a signature, decrypt a key, or steer a ceremony toward an origin other than the one displayed.
...
| **Clipboard auto-clear** | Copies use the native host's privacy-enhancing clipboard action where available, which applies a sensitivity hint and clears the clipboard again after `clipboardTimeout` seconds (see [Clipboard copy protection](#clipboard-copy-protection)). The browser-API fallback path does *not* auto-clear, in order to avoid holding a `clipboardRead` permission. |
...
| **Card entries bypass origin-matching** | Cards must work across many sites, so they default to `originBound: false` — visible on any origin rather than only their own. This is mitigated by `scope: "context"`, which hides them unless the page actually contains card form fields. Both defaults are overridable per-rule in `.parcel.json`. |
| **URL scopes apply at document load** | SPA `pushState` navigation does not re-evaluate in-page gates or cascades (the toolbar popup does reflect the live tab URL): a page loaded at a benign path can navigate onto a user-blacklisted path and keep its load-time features, for itself and embedded frames. The worst case is falling back to Parcel's default scope - its ordinary security model - and the scheme-level blacklist floor cannot be escaped this way. Closing the gap would need page-realm `history` hooks or a broad `webNavigation` permission, both considered worse than the gap itself. |

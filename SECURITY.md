...
| **URL scopes apply at document load** | A frame's scope rules are matched against its URL at load, and ancestor cascading uses the URLs recorded by passive `webRequest` observation; `history.pushState`-style SPA navigation does not re-evaluate in-page gates (the toolbar popup does reflect the live tab URL). A page that loaded at a benign path can therefore `pushState` onto a path a user's blacklist rule targets and keep its load-time features, for itself and its embedded frames. The worst case is falling back to Parcel's default scope, i.e. its ordinary security model; the scheme-level blacklist floor cannot be escaped this way because a scheme change requires a real navigation. Closing the gap would require page-realm `history` API hooks or a broad `webNavigation` permission, both considered worse than the gap itself. |

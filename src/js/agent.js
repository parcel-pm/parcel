...
export class Agent extends EventTarget {
    ...
    async #connect(port) {
        ...
        let tabId = null;
        let token = null;
        let httpAuthPort = false; // authenticated with a pending http-auth challenge token
        ...
        port.onMessage.addListener(async (message) => {
            try {
                if (port.name === "popup") {
                    if (
                        message?.action === "auth" &&
                        (this.#authorisedTokens.has(message.token) ||
                            message.token === "broadcast" ||
                            this.#pendingAuthCallbacks.has(message.token))
                    ) {
                        ...
                        // An http-auth token is a per-challenge random UUID stored in #pendingAuthCallbacks, generated by onAuthRequired.
                        // Its port may only ever decrypt with intent "http-auth" - form fills are barred for the port's whole lifetime.
                        if (message.token !== "broadcast" && !this.#pendingAuthCallbacks.has(message.token))
                            this.#authorisedTokens.delete(message.token);
                        authorised = true;
                        // Sticky: re-authenticating with another token must not lift the http-auth restriction.
                        httpAuthPort = httpAuthPort || this.#pendingAuthCallbacks.has(message.token);
                        token = message.token;
                ...
                if (message?.action === "http-auth-cancel") {
                    ...
                    if (this.#pendingAuthCallbacks.has(token)) {
                        this.#resolveAuthCallback(token, { cancel: true });
                    }
                    ...
                    if (httpAuthPort && message.intent !== "http-auth") {
    ...
    /**
     * Resolve a pending HTTP auth callback and clean up its timer.
     *
     * @since 1.0.6
     * @param {string} authToken - The per-challenge token generated by {@link #handleAuthRequired}.
     * @param {object} result - The result to pass to the asyncBlocking callback.
     * @returns {void}
     */
    #resolveAuthCallback(authToken, result) {
        const entry = this.#pendingAuthCallbacks.get(authToken);
        if (!entry) return;
        clearTimeout(entry.timer);
        this.#pendingAuthCallbacks.delete(authToken);
     ...
     *
     * The popup communicates via the background's popup port using a per-challenge
     * random token stored in {@link #pendingAuthCallbacks}, which restricts
     * decryption to intent "http-auth".
     *
     * @since 1.0.6
    ...
    async #handleAuthRequired(details, callback) {
        ...
        const abort = () => this.#resolveAuthCallback(authToken, {});
        ...
        try {
            ...
            hasContentScript = /^(https?|file):/.test(tabInfo.url || "");
        } catch {
            // Tab was closed between the 401 firing and this call
            abort();
            return;
        }
        ...
        if (hasContentScript) {
            try {
                await chrome.tabs.sendMessage(details.tabId, { action: "trigger-http-auth", token: authToken }, { frameId: 0 });
            } catch {
    ...
    async #getPublicSuffix(hostname) {
        ...
        for (let suffix = hostname; suffix.length; suffix = suffix.slice(suffix.indexOf(".") + 1)) {
            // exception rules resolve in place: their public suffix is the rule minus its leftmost
            // label; continuing the scan would fall past unlisted parents to a shorter rule

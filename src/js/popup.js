...
(async () => {
    ...
    const { initPasskeyPopup, initPasskeyConflict } = await import(chrome.runtime.getURL("/js/popup-webauthn.js"));
    ...
    if (mode === "passkey") {
        initPasskeyPopup({ tabPort, config, copyText, setStatus });
    } else if (mode === "passkey-conflict") {
        initPasskeyConflict({ tabPort });
    } else {

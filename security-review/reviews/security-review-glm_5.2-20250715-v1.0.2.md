...
- **F1 (LOW):** the `config` endpoint ships the full config (incl. the password-store home path) to unauthenticated integration ports. Reachable only under the compromised-extension model; worth fixing as defence-in-depth.
- **F2 (MEDIUM):** the broadcast fill path performs no destination-origin validation, so mid-decrypt navigation or popup-death can route a decrypted credential intended for one origin into a different-origin page, bypassing the only (advisory, popup-path-only) cross-origin control.
...
| T8 | Default burst (24) vs. refill (0.006667/s ≈ 24/hr) | Balance usability against exfiltration | Reasonable for sustained exfiltration; the *burst* provides little protection for small stores (see L-burst finding, consistent with prior L4 where maintainers judged typical stores larger). |
...
| T10 | Detectable by websites via `web_accessible_resources` + shadow shim | Acceptable fingerprint necessity | Accepted; findings.md L7 records the maintainers empirically verified each WAR entry is required. The reviewer did not re-litigate this. |
...
|----|----------|------------|-------|
| F1 | LOW | High | `config` endpoint returns the full config (incl. `passdir`/`realPassdir` → OS home path) to unauthenticated integration ports |
| F2 | MEDIUM | Medium | Broadcast fill path performs no destination-origin validation; mid-decrypt navigation or popup-death can route a decrypted credential to a different-origin page |

---

### F1 — `config` endpoint leaks the full config (incl. password-store home path) to unauthenticated integration ports
...
### F2 — Broadcast fill path performs no destination-origin validation; mid-decrypt navigation can exfiltrate a credential to a different origin
...
- **Port-name→action allow-list (M3) holds.** Under the non-compromised model a content-script port cannot reach `decrypt`/`match` (restricted to authorised popup ports), and `integration` ports are limited to `config`. The `auth` token is single-use (deleted after use).
...
**Coverage check for gaps:** The three subagents collectively covered the native host (signature/hash/whitelist/action/quoting/rate-limit/audit/framing), the extension runtime (ports/MV3/broadcast/postMessage/fill-target/storage/XSS), and config/packaging (manifest/CSP/plaintext/schema/selectors/packaging/signing/gitleaks). Cross-layer data-flow (host-injected `passdir` → extension config → integration endpoint) is exactly the seam the parallel split risked missing; the second-look explicitly traced it and produced F1 (with the corrected reachability). No layer was left unexamined, and the highest-risk native control (the whitelist exact-match) was independently re-verified by the reviewer.

---

## 8. Per-finding targeted follow-up

### F1 targeted follow-up — confirmed real, scope reaffirmed as LOW

Re-investigation: (a) confirmed `passdir`/`realPassdir` are injected by `src/parcel-host` (jq at the config-assembly point) and declared required/optional in `schema.js`; (b) confirmed the `config` handler returns `this.#config` wholesale and `integration` ports require no auth; (c) confirmed `integration.js` receives config into an isolated-world `Promise` and never writes it to page-readable space (only `postMessage`s `frameId`). **Outcome:** F1 is real as a gratuitous-info-disclosure/hygiene issue. Severity reaffirmed LOW (not a web-reachable attack; subsumed under compromised-extension damage class except for the marginal rate-limit-free OS-username correlator). The minimal-config-slice recommendation stands. No coupled issue found beyond the `realPassdir` path-obfuscation-defeat note already in the finding.

### F2 targeted follow-up — confirmed real, severity MEDIUM affirmed with caveats

Re-investigation: (a) confirmed `integration.js`'s broadcast `fill` handler has no origin guard (fills `msg.plaintext` into the best current root-frame target); (b) confirmed the agent broadcast fallback posts `{action:"fill", config, plaintext}` directly to `chrome.tabs.connect(tabId, {name:"broadcast", frameId:0})` with no origin check, gated on `token === "broadcast"` and a disconnected popup port; (c) confirmed the popup origin check is advisory-only (no `return`/abort after the `alert`). **Outcome:** F2 is real. The exploitability timing preconditions (slow decrypt + popup-port disconnect + cross-origin navigation into a fillable-target page) keep it at MEDIUM / Medium confidence rather than HIGH. **Coupled consideration:** the advisory-only nature of the *popup-path* origin check is itself borderline (a credential still flows to a mismatched origin after the user dismisses the alert); I folded this into F2's recommendation rather than opening a separate finding, since both stem from the same "fills proceed without a hard destination-origin gate" gap. The accepted autofill-phishing tradeoff (fill into a page the user is willingly interacting with) does **not** cover the F2 redirect variant, because the user's intended origin (A) differs from the exfiltration origin (B).

...
- **F18M** — Extension-page CSP omits `connect-src`/`frame-src`/`base-uri`; the documented "no network" rule could be *technically* enforced for the popup even though it cannot be for content scripts.
- **F19M** — MV3 service-worker restart loses authorisation state and re-`eval`s the signed host on every cold start; the lifecycle is undocumented and unhandled.
- **F20M** — `decrypt`/`match` actions on `agent.js` are not restricted to authorised popup ports, and the audited `origin` is forwarded verbatim from the message body instead of `port.sender.tab.url`. The host whitelist still bounds *which* entries are decryptable, but the per-popup authorisation gate and audit-log integrity can be bypassed by any extension context with a runtime port.
...
|----|-------|------|------|-----------|
| F18M | MED   | 8/10 | manifest/CSP | Extension-page CSP omits `connect-src 'none'`/`frame-src 'none'`/`base-uri 'self'`; no-network rule is only human-enforced even where it could be technical. |
| F19M | MED   | 7/10 | SW lifecycle | MV3 SW restart loses `#authorisedTokens`/`#config`/in-flight calls; re-`eval`s signed host on every cold start; no `onStartup`/`chrome.alarms` keepalive. |
| F20M | MED   | 7/10 | agent routing | `decrypt`/`match` reachable from any port name (not just authorised popup); `message.origin` forwarded to host & audit log without reconciliation against `port.sender.tab.url`. |
...
| F21T | LOW   | 8/10 | host config | Default `VALID_SIGNERS` includes both "backup-only" keys equally with primaries; contradicts CONSTITUTION §2.2 backup-key policy. |
| F22L | LOW   | 7/10 | host hash | `HOST_HASH` hashes a bash here-string (adds trailing newline); mismatches the `sha256sum src/parcel-host` basis documented in SECURITY.md/README. |
| F23T | LOW   | 7/10 | rate limit | Default `decryptBucket=24` ≥ typical store size; rate limiter provides ~zero burst protection in the default-allow-all case. |
| F24T | LOW   | 8/10 | audit log | `LOGFILE` path unvalidated; can point outside the constitution's "dedicated log file" intent (`/dev/null`, sensitive paths). |
| F25T | LOW   | 8/10 | audit log | Audit log file mode not pinned to 0600; default umask yields 0644 (entry paths + origins world-readable). |
| F26T | LOW   | 8/10 | WAR surface | 6 JS modules in `web_accessible_resources` need not be; "required by popup" rationale is incorrect for modules loaded via `import(chrome.runtime.getURL)` from extension contexts. |
| F27T | LOW   | 7/10 | storage | Fill-history keys are per-origin-named but globally readable from any content-script context via `chrome.storage.local.get(null)`; hashes of entry paths are brute-forceable. |
| F28T | LOW   | 8/10 | event forging | MAIN-world `shadow.js` `parcel-shadow-click` event and the `parcel-frame-id` `postMessage(..., "*")` are forgeable by the page; can open the popup / spoof frame id. |
| F29L| LOW   | 8/10 | string inj | `$SHA256` used unquoted in command position; benign today but fragile against paths with spaces. |
| F30L| LOW   | 8/10 | error leak | GPG status output included in `parcel_error` messages sent to the extension on signature failure. |
| F31L| LOW   | 7/10 | action dispatch | `action_$ACTION` dispatch ungated by regex; benign today but defence-in-depth allowlist would future-proof. |
| F32T| LOW   | 9/10 | crypto nitpick | `HOST_HASH` comparison is non-constant-time; no remote timing oracle exists, reported for completeness. |

---

## Detailed Findings

### F18M — Extension-page CSP does not technically enforce no-network
...
### F20M — `decrypt`/`match` not restricted to authorised popup ports; audited origin is attacker-controlled
...
### F22L — `HOST_HASH` hashes a bash here-string; mismatches documented basis
...
### F26T — `web_accessible_resources` exposes 6 JS modules unnecessarily
...
- **`HOST_HASH` off by default** — creates friction; opt-in with strong recommendation. Acceptable, given F22L above should be fixed so the opt-in is verifiable.
- **Absent `.parcel.json` reveals all entries** — an empty password store is not useful; restriction is opt-in. Acceptable for the audience, but F23T (burst capacity) materially weakens the mitigation for small stores.
- **Content script injected into `<all_urls>`** — required to detect forms before user interaction; the content script does not execute remote code or communicate externally. Acceptable; F18M would add technical enforcement for extension pages.
...
- **Extension detectable / fingerprintable** — acknowledged necessity; F26T would shrink the surface.
...
The findings with the highest confidence (8-9/10) are those I personally re-verified against the source (F18M, F19M, F20M, L1, F21T, F22L, F26T, F28T, F30L, F32T). The remaining LOW findings (F23T-F27T, F29L, F31L) rest on the subagents' reading, which I cross-checked against the cited file:line references and found accurate for every spot I re-examined. Where a subagent and my reading disagreed on severity (F20M), I have noted both perspectives.
...
1. **F20M** — Gate `decrypt`/`match` to authorised popup ports and derive the audited origin from `port.sender.tab.url`. This closes the audit-log-bypass path under the extension-compromise threat model.
2. **F18M** — Add `connect-src 'none'; frame-src 'none'; base-uri 'self';` to the extension-page CSP. Free, technically enforces the no-network rule for the popup.
3. **F19M** — Document (and optionally mitigate) the MV3 service-worker restart lifecycle.
4. **F22L** — Fix `HOST_HASH` to hash the exact script bytes so the documented pin is verifiable.
5. **F21T** — Default `VALID_SIGNERS` to primary keys only.
6. **F23T / F25T / L1** — Tighten default burst capacity, pin log file mode, and check config/log directory permissions.
